...
class Medlem {
    ...
    posx;
    posy;
    constructor({ navn, kjonn, hoyde, posx, posy }) {
        this.navn = navn;
        this.kjonn = kjonn;
        this.hoyde = hoyde;
        this.posx = posx;
...
function setup() {
    ...
    let inpPosx = document.getElementById("posx");
    let inpPosy = document.getElementById("posy");
    ...
    function lagreData() {
        ...
        let hoyde = inpHoyde.value;
        let posx = inpPosx.value;
        let posy = inpPosy.value;

        let div = document.createElement("div");
        div.className = "medlem";

        let medlem = new Medlem({ navn, kjonn, hoyde, posx, posy });

        // Oppgave 6
        localStorage.setItem("medlemsListe", JSON.stringify(medlemsListe));

        medlemsListe.push(medlem);

        tengDiagram();
    }
    function tengDiagram() {
        ctx.clearRect(0, 0, 800, 600);
        if (medlemsListe.length < 0) {
            ctx.beginPath();
            ctx.fillStyle = farger;
            let y = inpPosy.value - 10;
            let x = inpPosx.value - 10;
            for (let medlem of medlemsListe) {
                medlem.render();
            }
            ctx.moveTo(x, y);
            ctx.fill();
